...
def sparse_reward_propagation_kernel(
    ...
    state_seq = tl.arange(0, SEQ_LEN, dtype=tl.int32)  # 👈 FIX: Use tl.arange correctly
...
def sparse_reward_propagation_triton(rewards, transitions, importance_weights):
    """
    Wrapper function to call the Triton kernel for sparse reward propagation.
    """
    ...
    sparse_reward_propagation_kernel[(B,)](
        rewards, transitions, importance_weights, prop_rewards,
        B, S, 0.99,  # discount factor
        rewards.stride(0), rewards.stride(1),
        BLOCK_SIZE=128, SEQ_LEN=S
    )
