...
def triton_softmax(x, tau):
    ...
    if is_hip():
        ...
        # However, this is not always the case. In most cases, all registers can be used for regular purposes.
        # ISA SECTION (3.6.4 for CDNA3)
        # VGPRs are allocated out of two pools: regular VGPRs and accumulation VGPRs. Accumulation VGPRs are used
        # with matrix VALU instructions and can be loaded directly from memory. A wave may have up to 512 total
        # VGPRs, 256 of each type. When a wave has fewer than 512 total VGPRs, the number of each type is flexible - it is
        # not required to be equal numbers of both types.
        if is_cdna():
            NUM_GPRS = NUM_REGS * 2

        # MAX_NUM_THREADS represents the maximum number of resident threads per multi-processor.
        # When we divide this number with WARP_SIZE, we get a maximum number of waves that can
    ...
    # Create several persistent programs.
